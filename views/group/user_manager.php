...
<?php if ($groupUser->role == GroupUser::ROLE_APPLICATION): ?>
    <?= Html::a(Yii::t('app','Apply to join'), ['/group/user-update', 'id' => $groupUser->id, 'role' => 1], ['class' => 'btn btn-success']); ?>
...
<?php elseif ($groupUser->role == GroupUser::ROLE_MEMBER && $model->getRole() == GroupUser::ROLE_LEADER): ?>
    <?= Html::a(Yii::t('app','Set administrator'), ['/group/user-update', 'id' => $groupUser->id, 'role' => 4], ['class' => 'btn btn-default']); ?>
<?php elseif ($groupUser->role == GroupUser::ROLE_MANAGER && $model->getRole() == GroupUser::ROLE_LEADER): ?>
    <?= Html::a(Yii::t('app','Set normal member'), ['/group/user-update', 'id' => $groupUser->id, 'role' => 5], ['class' => 'btn btn-default']); ?>
<?php endif; ?>
